...
//     * Set crop web options:  -a -wfqpn "c:\gobii-config-test\testconfig.xml" -c "barcrop" -stW -soH "foohost" -soN 8080 -soU "foo userr" -soP "foo password" -soR "foo-web"
    ...
    public void testSetKDCServerOptions() throws Exception {
        ...
        Assert.assertTrue("The start resource does not not match",
                configSettings.getGlobalServer(ServerType.KDC).getPath(ServerBase.KDCResource.QC_START).equals(resourceQCStart));

        Assert.assertTrue("The status resource does not not match",
                configSettings.getGlobalServer(ServerType.KDC).getPath(ServerBase.KDCResource.QC_STATUS_).equals(resourceQCStatus));

        Assert.assertTrue("The download resource does not not match",
                configSettings.getGlobalServer(ServerType.KDC).getPath(ServerBase.KDCResource.QC_DOWNLOAD).equals(resourceQCDownload));

        Assert.assertTrue("The purge resource does not not match",
                configSettings.getGlobalServer(ServerType.KDC).getPath(ServerBase.KDCResource.QC_PURGE).equals(resourceQCPurge));
